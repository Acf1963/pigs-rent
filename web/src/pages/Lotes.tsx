...
export default function LotesPage() {
  ...
  const initialForm = {
    codigoLote: '', tipo: 'SUÍNO', raca: '', quantidade: '', fornecedor: '',
    dataEntrada: new Date().toISOString().split('T')[0],
    pesoInicialMedio: '', pesoFinalTransporte: '', custoTransporte: '', custoAquisicao: '',
  };
  ...
  const getResumoTotal = () => {
    let bStock = 0, sStock = 0, bLotes = 0, sLotes = 0, bAbates = 0, sAbates = 0, bCarc = 0, sCarc = 0;
    lotes.forEach(l => {
      ...
      const carc = nA - nV;
      if (String(l.tipo).toUpperCase().includes('BOVINO')) { bStock += vivo; bLotes++; bAbates += nA; bCarc += carc; }
      else { sStock += vivo; sLotes++; sAbates += nA; sCarc += carc; }
    });
    return {
      totalAbates: abates.length, totalLotes: bLotes + sLotes, totalCarcacas: bCarc + sCarc, stockGlobal: bStock + sStock,
  ...
  const exportToExcel = () => {
    const dataMain = lotes.map(l => ({
      'Cód. Lote': l.codigoLote, 'Tipo': l.tipo, 'Raça': l.raca, 'Qtd': l.quantidade,
      'Fornecedor': l.fornecedor, 'Data Entrada': l.dataEntrada, 'Peso Médio': l.pesoInicialMedio,
      'Peso Final Transp.': l.pesoFinalTransporte, 'Custo Transp.': l.custoTransporte, 'Custo Aquisição': l.custoAquisicao
    }));
  ...
  return (
    /* AJUSTE: h-[calc(100vh-80px)] reserva espaço para a navbar e justify-between segura o rodapé */
    <div className="w-full h-[calc(100vh-80px)] flex flex-col justify-between bg-[#0a0f18] text-gray-200 font-sans overflow-hidden p-4">
      ...
      {/* HEADER E FORMULÁRIO (Agrupados para não serem empurrados individualmente) */}
        ...
        <div className="bg-[#111827]/50 p-6 rounded-2xl border border-gray-800/50 mb-4 shadow-xl">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div className="space-y-2">
              ...
              <input required className="w-full bg-black/40 border border-gray-700 rounded-xl p-3 text-white outline-none focus:border-blue-500 uppercase font-bold" value={formData.codigoLote} onChange={e => setFormData({...formData, codigoLote: e.target.value})} />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <input type="number" required className="w-full bg-black/40 border border-gray-700 rounded-xl p-3 text-white outline-none focus:border-blue-500 font-bold" value={formData.quantidade} onChange={e => setFormData({...formData, quantidade: e.target.value})} />
            </div>
            <div className="flex gap-2">
      ...
      {/* TABELA - O SCROLL FICA APENAS AQUI */}
          ...
          <table className="min-w-[2100px] w-full text-left text-[11px] border-separate border-spacing-0">
            <thead className="bg-[#0f1522] text-gray-500 font-black uppercase sticky top-0 z-20">
              <tr>
                ...
                <th className="p-4 border-b border-gray-800 bg-[#0f1522]">P. Inicial Médio</th>
      ...
      {/* RODAPÉ - SEMPRE VISÍVEL NO FUNDO */}
      <div className="p-4 bg-[#0c121d] border border-gray-800 rounded-2xl grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0 shadow-2xl">
        <div className="bg-[#1a2233]/50 p-4 rounded-xl border border-gray-800 flex flex-col justify-between h-24">
          <span className="text-[10px] text-blue-400 font-black uppercase block tracking-widest">BOVINOS EM STOCK</span>
          <span className="text-4xl font-black text-white italic">{resumo.bovinos.vivos}</span>
        </div>
        <div className="bg-[#1a2233]/50 p-4 rounded-xl border border-gray-800 flex flex-col justify-between h-24">
          <span className="text-[10px] text-pink-400 font-black uppercase block tracking-widest">SUÍNOS EM STOCK</span>
          <span className="text-4xl font-black text-white italic">{resumo.suinos.vivos}</span>
        </div>
        <div className="bg-[#1a2233]/50 p-4 rounded-xl border border-gray-800 flex flex-col justify-center h-24">
          <span className="text-[10px] text-orange-500 font-black uppercase block tracking-widest mb-1">TOTAL ABATES</span>
          <span className="text-4xl font-black text-white italic">{resumo.totalAbates}</span>
        </div>
        <div className="bg-emerald-500/10 p-4 rounded-xl border border-emerald-500/20 text-right flex flex-col justify-center h-24">
          <span className="text-[10px] text-emerald-500 font-black uppercase block tracking-widest mb-1">STOCK VIVO REAL</span>
          <span className="text-4xl font-black text-emerald-500 italic">{resumo.stockGlobal}</span>
        </div>
      </div>
