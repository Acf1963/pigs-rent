...
import { 
  Users, Plus, Trash2, 
  UploadCloud, FileSpreadsheet, FileText, Square, CheckSquare 
} from 'lucide-react';

import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import * as XLSX from 'xlsx'; // Importação para o Excel
...
export default function LotesDetalhePage() {
  ...
  // --- EXPORTAÇÃO EXCEL ---
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(animais.map(a => ({
      Lote: a.loteId,
      Brinco: a.brinco,
      Peso_Kg: a.pesoEntrada,
      Raca: a.raca || 'N/A',
      Data_Entrada: a.dataEntrada
    })));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Animais");
    XLSX.writeFile(workbook, `Relatorio_Lote_${selectedLote || 'Geral'}.xlsx`);
  };
  ...
  const exportToPDF = () => {
    ...
    autoTable(docPDF, {
      head: [["BRINCO", "LOTE", "RAÇA", "PESO (KG)"]],
      body: animais.map(a => [a.brinco, a.loteId, a.raca || '---', `${a.pesoEntrada} Kg`]),
    ...
    <div className="h-[calc(100vh-110px)] flex flex-col space-y-4 overflow-hidden p-2">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight">ANIMAIS (ID)</h1>
        </div>
      ...
      <div className="bg-[#161922] rounded-2xl border border-slate-800/50 p-4 md:p-6 shrink-0">
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:flex md:flex-nowrap gap-4 items-end">
          <div className="col-span-2 md:flex-1 space-y-2">
            <label className="text-[9px] font-black text-slate-500 uppercase px-1">Lote ID</label>
            <select 
              className="w-full bg-[#0d0f14] border border-slate-800 p-3 rounded-lg text-white text-xs font-bold outline-none uppercase focus:border-cyan-500"
              value={selectedLote}
          ...
          <button type="submit" className="col-span-2 md:w-auto bg-[#009dc4] hover:bg-cyan-500 text-white font-black text-[11px] px-8 py-3.5 rounded-xl uppercase flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/20">
            <Plus size={18} /> Gravar
          </button>
        </form>
      </div>
      ...
      <div className="bg-[#161922] rounded-2xl border border-slate-800/50 shadow-2xl flex flex-col flex-1 min-h-0 overflow-hidden">
        <div className="overflow-y-auto flex-1 custom-scrollbar overflow-x-auto"> 
          <table className="w-full text-left text-[11px] border-separate border-spacing-0 min-w-[600px]">
            <thead className="bg-[#11141d] text-slate-500 font-black uppercase text-[9px] sticky top-0 z-10">
              <tr>
                ...
                <th className="p-4 border-b border-slate-800/50">RAÇA / OBS.</th>
            ...
            <tbody className="divide-y divide-slate-800/20">
              {animais.map((a) => (
                <tr key={a.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="p-4 text-slate-500 font-bold uppercase">{a.raca || 'PADRÃO'}</td>
                  <td className="p-4 text-center">
        ...
        {/* RODAPÉ FIXO */}
        <div className="p-4 bg-[#0d0f14] border-t border-slate-800/50 flex justify-between items-center px-6 shrink-0">
          <div className="flex gap-10">
            <div className="flex flex-col">
              <span className="text-[8px] font-black text-slate-500 uppercase">Total Cabeças</span>
              <span className="text-sm font-black text-white">{totalCabecas} UN.</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[8px] font-black text-slate-500 uppercase">Média Peso</span>
              <span className="text-sm font-black text-cyan-500">{mediaPeso} KG</span>
            </div>
          </div>
